...
3. **工作区与 Git 状态集成**
   ...
   - **推送/拉取指标**: 实时计算并展示 `⇡` (待推送 Commit 数) 和 `⇣` (待拉取 Commit 数)（仅在数量大于 0 时展示，无待推送或待拉取 Commit 时隐藏）。
   - **未提交修改数 (📝)**: 统计当前未提交文件的数量（仅在大于 0 时显示，即无未提交修改时隐藏）。
4. **Token 上下文用量监控 (⚡)**
...
```powershell
# PowerShell
Get-Content "$HOME\.gemini\antigravity-cli\last_payload.json" -Raw | bash "/path/to/statusline.sh"
```

> **注意**：在 Windows PowerShell 中运行上述测试命令时，请确保 `bash` 已被添加到您的系统环境变量 `PATH` 中。如果未添加，请直接在 Git Bash 中运行测试。
